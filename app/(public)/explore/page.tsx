...
export default function ExplorePage() {
  ...
  // Fetch events
  const featuredEvents = useQuery(api.explore.getFeaturedEvents, { limit: 3 });

  const localEvents = useQuery(api.explore.getEventsByLocation, {
    city: currentUser?.location?.city || "Gurugram",
    state: currentUser?.location?.state || "Haryana",
    limit: 4,
  });

  const popularEvents = useQuery(api.explore.getPopularEvents, { limit: 6 });

  const categoryCounts = useQuery(api.explore.getCategoryCounts);

  // Calculate loading states
  const loadingFeatured = featuredEvents === undefined;
  const loadingLocal = localEvents === undefined;
  const loadingPopular = popularEvents === undefined;
  const isLoading = loadingFeatured || loadingLocal || loadingPopular;
